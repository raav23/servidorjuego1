...
//ESTO SE EJECUTARA CUANDO EL CLIENTE APENAS SE CONECTE AL SERVER////////////////////////////////////////////////////////////////////
			console.log("Se conecto cliente: " +socket.name+", desde: "+socket.remoteAddress);
			...
			numDelSocketEnSala++;
			console.log("numero del socket en sala: "+numDelSocketEnSala);
			...
			socket.turno=false;//***** Para controlar los 15 segundos que tiene el jugador en su turno para "Darle"
			...
			if(arraySockets[socket.sala]==null){ 
				...
				console.log("Creado nuevo array dentro de arraySockets[indice]: "+socket.sala);
			...
			//SI YA HA SIDO CREADA:
				...
				if(arraySockets[socket.sala][0]=="NULO"){
					...
					console.log("Se introdujo socket dentro del arraySockets[sala][0]");
					arraySockets[socket.sala][0].write("0");
					socket.numEnSala=0;
						...
						var index = arraySockets[socket.sala].indexOf("NULO");
			   			arraySockets[socket.sala].splice(index, 1,socket);
						console.log("Se introdujo socket dentro del arraySockets[sala][1]");
						...
						socket.numEnSala=1;
							...
							var index = arraySockets[socket.sala].indexOf("NULO");
				   			arraySockets[socket.sala].splice(index, 1,socket);
							console.log("Se introdujo socket dentro del arraySockets[sala][2]");
							...
							socket.numEnSala=2;
								...
								arraySockets[socket.sala].push(socket);
							console.log("Se introdujo otro socket dentro del arraySockets[indice]: "+socket.sala);
						...
						if(arraySockets[socket.sala].length==4){
							...
							setTimeout(function(){
								...
								try {
							        ...
							        arraySockets[socket.sala][0].setTimeout(15000,function(){
			...
			if(arrayNumeros[socket.sala]==null){
					...
					switch (data) {
							case "10":
								...
								console.log("El primer jugador introdujo: "+data+", en la propiedad n1.")	;
								break;
							case "11":
						...
						//Si no hay nada en .n2
							...
							switch (data) {
									case "10":
										...
										console.log("El Segundo jugador introdujo: "+data+", en la propiedad n2.")	
										break;
									case "11":
							...
							//Si no hay nada en .n3
								...
								switch (data) {
									case "10":
										...
										console.log("El tercer jugador introdujo: "+data+", en la propiedad n3.")
										break;
									case "11":
			...
			setTimeout(function(){
				...
				try {
					...
					if(arraySockets[socket.sala][1].turno==true){
						...
						//Si pasan 15 segundo y el socket no envia ningun numero, enviamos automaticamente 1
						arraySockets[socket.sala][1].setTimeout(15000,function(){
							console.log("Efectivamente socket 2 cumplio 15 segundos fuera")
							
							
						});
						//socket 2turno=false
						arraySockets[socket.sala][1].turno=false;
						

					}else if(arraySockets[socket.sala][2].turno==true){
						//Activamoss tiempo de 20 segundos al socket3
						arraySockets[socket.sala][2].setTimeout(15000,function(){
							console.log("Efectivamente socket 3 cumplio 15 segundos fuera")
							//arraySockets[socket.sala][2].write("closed2");

						});
					   //Eliminamos tiempo de espera de 20 segundos  al socket 2
						arraySockets[socket.sala][1].setTimeout(0);
						//socket3.turno=false;
						arraySockets[socket.sala][2].turno=false;

					}else if(arraySockets[socket.sala][3].turno==true){
					//Activamoss tiempo de 20 segundos al socket4
						arraySockets[socket.sala][3].setTimeout(15000,function(){
							console.log("Efectivamente socket 4 cumplio 15 segundos fuera")
							

						});
					   //Eliminamos tiempo de espera de 15 segundos  al socket 3
						arraySockets[socket.sala][2].setTimeout(0);
...
//>4	//Pero si es el 4to socket de la sala entonces:	
		//AQUI ES DONDE SE EJECUTAN LOS CALCULOS: QUIEN PIERDE; CUANTO PIERDE; SE REDISTRIBUYE EL DINERO.....!!!!!!!!!!!!!!!!
			...
			switch (data) {
					...
					console.log("El cuarto jugador introdujo: "+data+", en la propiedad n4.")	;
					break;
			...
			console.log("socket.ronda: "+socket.ronda);

			//Si es el final de la 4ta ronda cerramos la sala con el cierre del cliente
			if(socket.ronda==5){
			//Despues de 4 segundo
			setTimeout(function(){
				console.log("Aqui enviamos algo para que el cliente se cierre")
				try {
					arraySockets[socket.sala].forEach(function(elemento){							
...
var j = schedule.scheduleJob('0 15 19 * * *', function(){
//Datos del correo
